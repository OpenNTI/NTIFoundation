...
-(BOOL)writeLinkAttributes: (NSDictionary*)newAttributes
{
	...
	if(		[attachment respondsToSelector: @selector(attachmentCell)]
	   &&	[(id)[attachment attachmentCell] respondsToSelector: @selector(image)] ) {
		OATextAttachmentCell* cell = [attachment attachmentCell];
		UIImage* image = [(id)cell image];
		if( !CGSizeEqualToSize( image.size, cell.cellSize ) ) {
			//Resize the image to match what it was displayed as.
			//TODO: Is there a better way?
			UIGraphicsBeginImageContextWithOptions( cell.cellSize, NO, 0.0 );
			//No need to put a scale on the context, the image does that.
			[image drawInRect: CGRectMake( 0, 0, cell.cellSize.width, cell.cellSize.height )];
		...
		OFDataBufferAppendCString( self->dataBuffer,
								  [[pngData base64String] UTF8String] );
		OFDataBufferAppendCString( self->dataBuffer, "\" />" );
	}
...
-(void)buildColorTable;
{
	...
	NTIHTMLColorTableEntry* defaultColorEntry 
		= [[NTIHTMLColorTableEntry alloc] initWithColor: (id)[[UIColor blackColor] CGColor]];
