...
static CGColorRef parseColor( NSString* attribute )
{
	...
	if( [@"black" isEqual: attribute] ) {
		...
		color = [UIColor colorWithRed: [[components firstObject] floatValue] / 255.0f
								green: [[components secondObject] floatValue] / 255.0f
								 blue: [[components lastObject] floatValue] / 255.0f
								alpha: 1.0].CGColor;
	}
		...
		color = [UIColor colorWithRed: [r hexValue] / 255.0f
	...
	attributes: (NSDictionary*)attributeDict
{
	...
	if( [@"a" isEqual: elementName] ) {
		...
		NTI_RELEASE( self->currentImage );
		NSString* src = [attributeDict objectForKey: @"src"];
		self->currentImage = [[self loadImage: src] retain];
	}
...
#undef POPIF
	...
	//or BODY, if present, so our stack stays in sync.
		...
		NTIImageAttachmentCell* cell = [[NTIImageAttachmentCell alloc]
										initWithImage: self->currentImage
		...
		[self->attrBuffer appendString: [NSString stringWithCharacters: &attachmentCharacter
																length: 1]
							attributes: linkDict];
...
{
	NTIHTMLReader* r = [[NTIHTMLReader alloc] initWithHTML: htmlString];	
